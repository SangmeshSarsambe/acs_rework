...
public class StreamManager {
    ...
    private static void launchVlc(String ip, int port) {
        try {
            ...
            // In debug mode — inherit server console so VLC output is visible
            // In normal mode — discard VLC output, no terminal noise
            if (SimpleServer.DEBUG) {
                pb.inheritIO();
            } else {
            ...
            e.printStackTrace();
            ErrorDialog.warn("Stream Error",
                    "Failed to launch VLC stream viewer.\n" + e.getMessage());
            vlcProcess = null;
        }
    }
}